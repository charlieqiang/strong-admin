...
import com.strong.system.component.file.oss.FileOssClient;
...
public class FileServiceImpl implements FileService {
    ...
    @Autowired
    private FileOssClient fileOssClient;
    ...
    public String upload(MultipartFile file) {
        ...
        try {
            ...
            fileOssClient.uploadOssFile(uploadFilename, inputStream);
        } catch (Exception e) {
            log.error("文件上传出现异常，文件名：[{}]，异常信息：[{}]", filename, e.getMessage(), e);
            throw new CustomizeException("文件上传异常，请联系管理员");
        }
        return fileOssClient.getAssertFileUrl(uploadFilename);
    }

    @Override
    public void download(String date, String filename, HttpServletResponse response) {
        String key = date + "/" + filename;
        try (InputStream inputStream = fileOssClient.getOssFileInputStream(key)) {
